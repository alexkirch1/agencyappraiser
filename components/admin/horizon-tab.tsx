          ...
          <div className="mb-10 border-b border-border pb-8">
            ...
            {comm.loaded && comm.data.length > 0 && policy.loaded && (() => {
              ...
              return (
                ...
                {/* ── TEMPORARY DEBUG PANEL ── remove when gap analysis is complete ── */}
                {(() => {
                  // ── Build raw-to-normalized lookup maps ───────────────────────────
                  // rawPdfByNorm: normalized PDF key → first raw PDF policy string seen
                  const rawPdfByNorm = new Map<string, string>()
                  comm.data.forEach(c => {
                    const norm = normalizePolicy(c.policy_number)
                    if (!rawPdfByNorm.has(norm)) rawPdfByNorm.set(norm, c.policy_number)
                  })

                  // commRowByNorm: normalized PDF key → CommItem (for client name / $ lookup)
                  const commRowByNorm = new Map<string, typeof comm.data[0]>()
                  comm.data.forEach(c => {
                    const norm = normalizePolicy(c.policy_number)
                    if (!commRowByNorm.has(norm)) commRowByNorm.set(norm, c)
                  })

                  // rawEzByNorm: normalized EZLynx key → original EZLynx policy string
                  const rawEzByNorm = new Map<string, string>()
                  ezlynxList.forEach(e => {
                    const norm = normalizePolicy(e.policyNumber)
                    if (!rawEzByNorm.has(norm)) rawEzByNorm.set(norm, e.policyNumber)
                  })

                  // ── Matched samples (Pass 1/2/3) — show raw PDF key + norm + EZLynx ─
                  const matchedSamples: Array<{
                    rawPdf: string; normPdf: string; normEz: string; rawEz: string
                    client: string; pass: number
                  }> = []
                  for (const normEz of matchResult.matchedPolicies) {
                    if (matchedSamples.length >= 5) break
                    // Find the comm row that matched this EZLynx key
                    // Try direct key first, then find via rowPassMap traversal
                    let foundNormPdf: string | undefined
                    for (const [rawPdf] of rawPdfByNorm) {
                      if (normalizePolicy(rawPdf) === normEz) { foundNormPdf = normalizePolicy(rawPdf); break }
                    }
                    // Fallback: find via rowPassMap (raw pdf key stored there)
                    if (!foundNormPdf) {
                      for (const [rawPdfKey] of matchResult.rowPassMap) {
                        const np = normalizePolicy(rawPdfKey)
                        if (np === normEz) { foundNormPdf = np; break }
                      }
                    }
                    const normPdf = foundNormPdf ?? normEz
                    const rawPdf = rawPdfByNorm.get(normPdf) ?? normPdf
                    const client = commRowByNorm.get(normPdf)?.client_name ?? ezlynxList.find(e => normalizePolicy(e.policyNumber) === normEz)?.clientName ?? "—"
                    const pass = matchResult.rowPassMap.get(rawPdf) ?? matchResult.rowPassMap.get(normPdf) ?? 1
                    matchedSamples.push({ rawPdf, normPdf, normEz, rawEz: rawEzByNorm.get(normEz) ?? normEz, client, pass })
                  }

                  // ── Unmatched samples — PDF rows with no EZLynx match ─────────────
                  const unmatchedSamples: Array<{
                    rawPdf: string; normPdf: string; client: string; comm: number; premium: number
                  }> = []
                  for (const c of comm.data) {
                    if (unmatchedSamples.length >= 10) break
                    const normPdf = normalizePolicy(c.policy_number)
                    // A comm row is unmatched if its raw policy_number is NOT in rowPassMap
                    if (!matchResult.rowPassMap.has(c.policy_number) && !matchResult.rowPassMap.has(normPdf)) {
                  ...
                  // ── Gap analysis — structural pattern comparison ───────────────────
                  const pdfPatterns = unmatchedSamples.map(s => ({
                    raw: s.rawPdf,
                    norm: s.normPdf,
                    digitOnly: /^\d+$/.test(s.normPdf),
                    len: s.normPdf.length,
                    alphaPrefix: s.normPdf.match(/^[A-Z]+/)?.[0] ?? "",
                  }))
                  const ezPatterns = ezSamples.map(s => ({
                    norm: s.norm,
                    digitOnly: /^\d+$/.test(s.norm),
                    len: s.norm.length,
                    alphaPrefix: s.norm.match(/^[A-Z]+/)?.[0] ?? "",
                  }))
                  const pdfAvgLen = pdfPatterns.length ? Math.round(pdfPatterns.reduce((s, p) => s + p.len, 0) / pdfPatterns.length) : 0
                  const ezAvgLen  = ezPatterns.length  ? Math.round(ezPatterns.reduce((s, p) => s + p.len, 0) / ezPatterns.length)  : 0
                  const pdfDigitPct = pdfPatterns.length ? Math.round(pdfPatterns.filter(p => p.digitOnly).length / pdfPatterns.length * 100) : 0
                  const ezDigitPct  = ezPatterns.length  ? Math.round(ezPatterns.filter(p => p.digitOnly).length / ezPatterns.length * 100)  : 0
                  const pdfPrefixes = [...new Set(pdfPatterns.map(p => p.alphaPrefix).filter(Boolean))].slice(0, 6)
                  const ezPrefixes  = [...new Set(ezPatterns.map(p => p.alphaPrefix).filter(Boolean))].slice(0, 6)

                  return (
                    <div className="mt-4 rounded-lg border-2 border-amber-400 bg-amber-50/40 p-4 dark:bg-amber-900/10">
                      <div className="mb-3 flex items-center gap-2">
                        <span className="rounded bg-amber-400 px-2 py-0.5 text-[10px] font-bold uppercase text-amber-900">Debug — Matching Pipeline Audit</span>
                        <span className="text-[10px] text-muted-foreground">Remove this panel once gap analysis is complete</span>
                      </div>

                      {/* 1. Pipeline Breakdown */}
                      <section className="mb-4">
                        <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-400">1. Pipeline Breakdown</p>
                        <ol className="space-y-0.5 font-mono text-[11px] text-foreground">
                          <li>1. PDF text lines → <strong>parsePdfCommissionRow()</strong> → rows where commission &gt; 0 AND foundPol truthy</li>
                          <li>2. Summary blacklist filter: TOTAL / SUBTOTAL / DEPOSIT / REPORT / FEES / BALANCE / STATEMENT / PAGE / SUMMARY / OTHER COMMS / EZ COMMS / COMMISSIONS → null</li>
                          <li>3. Raw PDF policy string → <strong>normalizePolicy()</strong> → strip non-alphanumeric, strip trailing 00/01/000 suffix if base ≥6 chars, strip leading zeros on digit-only strings</li>
                          <li>4. EZLynx CSV rows → same <strong>normalizePolicy()</strong> applied to policy column → ezlynxNormSet</li>
                          <li>5. <strong>matchPolicies()</strong> Pass 1 — exact normalized key match</li>
                          <li>6. Pass 2 — substring containment ≥6 chars (handles sub-policy / master-policy splits)</li>
                          <li>7. Pass 3 — fuzzy client name (≥4-char word overlap) + premium within ±15%</li>
                          <li>8. <span className="text-success font-bold">{matchResult.matchedPolicies.size} EZLynx policies matched</span> · <span className="text-warning font-bold">{matchResult.unmatchedPolicies.size} unmatched</span> · Total comm rows: {comm.data.length}</li>
                        </ol>
                      </section>

                      {/* 2. Matched Samples */}
                      <section className="mb-4">
                        <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-400">2. Matched Samples (up to 5)</p>
                        {matchedSamples.length === 0
                          ? <p className="font-mono text-[11px] text-muted-foreground">No matched records found — check that both files are loaded.</p>
                          : (
                            <div className="overflow-x-auto">
                              <table className="w-full min-w-[640px] font-mono text-[11px]">
                                <thead>
                                  <tr className="border-b border-amber-200 dark:border-amber-800">
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Raw PDF Policy</th>
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Norm PDF Key</th>
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Norm EZLynx Key</th>
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Raw EZLynx</th>
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Client</th>
                                    <th className="pb-1 text-left font-semibold text-muted-foreground">Pass</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {matchedSamples.map((s, i) => (
                                    <tr key={i} className="border-b border-amber-100 dark:border-amber-900/40">
                                      <td className="py-0.5 pr-3 text-foreground">{s.rawPdf}</td>
                                      <td className="py-0.5 pr-3 text-success">{s.normPdf}</td>
                                      <td className="py-0.5 pr-3 text-success">{s.normEz}</td>
                                      <td className="py-0.5 pr-3 text-muted-foreground">{s.rawEz}</td>
                                      <td className="py-0.5 pr-3 truncate max-w-[160px] text-foreground">{s.client}</td>
                                      <td className="py-0.5 text-primary font-bold">P{s.pass}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                      ...
                      {/* 3. Unmatched Samples */}
                      <section className="mb-4">
                        <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-400">3. Unmatched PDF Rows (up to 10)</p>
                        {unmatchedSamples.length === 0
                          ? <p className="font-mono text-[11px] text-muted-foreground">All comm rows matched — or no data loaded.</p>
                          : (
                            <div className="overflow-x-auto">
                              <table className="w-full min-w-[640px] font-mono text-[11px]">
                                <thead>
                                  <tr className="border-b border-amber-200 dark:border-amber-800">
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Raw PDF Policy</th>
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Normalized Key</th>
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Client Name</th>
                                    <th className="pb-1 pr-3 text-right font-semibold text-muted-foreground">Comm $</th>
                                    <th className="pb-1 text-right font-semibold text-muted-foreground">Premium $</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {unmatchedSamples.map((s, i) => (
                                    <tr key={i} className="border-b border-amber-100 dark:border-amber-900/40">
                                      <td className="py-0.5 pr-3 text-warning">{s.rawPdf}</td>
                                      <td className="py-0.5 pr-3 text-foreground">{s.normPdf}</td>
                                      <td className="py-0.5 pr-3 truncate max-w-[160px] text-muted-foreground">{s.client}</td>
                                      <td className="py-0.5 pr-3 text-right text-foreground">{formatCurrency(s.comm)}</td>
                                      <td className="py-0.5 text-right text-muted-foreground">{formatCurrency(s.premium)}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                      ...
                      {/* 4. EZLynx Samples */}
                      <section className="mb-4">
                        <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-400">4. EZLynx Policy Samples (up to 10)</p>
                        {ezSamples.length === 0
                          ? <p className="font-mono text-[11px] text-muted-foreground">No EZLynx CSV loaded.</p>
                          : (
                            <div className="overflow-x-auto">
                              <table className="w-full min-w-[480px] font-mono text-[11px]">
                                <thead>
                                  <tr className="border-b border-amber-200 dark:border-amber-800">
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Raw EZLynx Policy #</th>
                                    <th className="pb-1 pr-3 text-left font-semibold text-muted-foreground">Normalized Key</th>
                                    <th className="pb-1 text-left font-semibold text-muted-foreground">Client</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {ezSamples.map((s, i) => (
                                    <tr key={i} className="border-b border-amber-100 dark:border-amber-900/40">
                                      <td className="py-0.5 pr-3 text-foreground">{s.raw || <span className="text-muted-foreground italic">empty</span>}</td>
                                      <td className="py-0.5 pr-3 text-primary">{s.norm || <span className="text-muted-foreground italic">—</span>}</td>
                                      <td className="py-0.5 truncate max-w-[200px] text-muted-foreground">{s.client}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
                      ...
                      {/* 5. Gap Analysis */}
                      <section>
                        <p className="mb-1.5 text-[11px] font-bold uppercase tracking-widest text-amber-700 dark:text-amber-400">5. Gap Analysis</p>
                        <div className="rounded border border-amber-200 bg-white/60 p-3 font-mono text-[11px] dark:bg-black/20">
                          <p className="mb-1 font-semibold text-foreground">Unmatched PDF key structure ({unmatchedSamples.length} sampled):</p>
                          <p className="text-muted-foreground">  Avg key length: <span className="text-foreground font-bold">{pdfAvgLen}</span> chars &nbsp;|&nbsp; Digit-only: <span className="text-foreground font-bold">{pdfDigitPct}%</span> &nbsp;|&nbsp; Alpha prefixes: <span className="text-foreground font-bold">{pdfPrefixes.length ? pdfPrefixes.join(", ") : "none"}</span></p>
                          <p className="mb-2 text-muted-foreground">  Sample norms: {unmatchedSamples.slice(0, 5).map(s => s.normPdf).join(" · ")}</p>
                          <p className="mb-1 font-semibold text-foreground">EZLynx key structure ({ezSamples.length} sampled):</p>
                          <p className="text-muted-foreground">  Avg key length: <span className="text-foreground font-bold">{ezAvgLen}</span> chars &nbsp;|&nbsp; Digit-only: <span className="text-foreground font-bold">{ezDigitPct}%</span> &nbsp;|&nbsp; Alpha prefixes: <span className="text-foreground font-bold">{ezPrefixes.length ? ezPrefixes.join(", ") : "none"}</span></p>
                          <p className="text-muted-foreground">  Sample norms: {ezSamples.slice(0, 5).map(s => s.norm).join(" · ")}</p>
                          <p className="mt-2 border-t border-amber-200 pt-2 font-semibold text-amber-700 dark:border-amber-700 dark:text-amber-400">
                            {pdfAvgLen > 0 && ezAvgLen > 0 && Math.abs(pdfAvgLen - ezAvgLen) >= 3
                              ? `Length mismatch: PDF keys avg ${pdfAvgLen} chars vs EZLynx avg ${ezAvgLen} chars — likely sub-policy vs master policy format.`
                              : pdfDigitPct !== ezDigitPct
                              ? `Format mismatch: ${pdfDigitPct}% of unmatched PDF keys are digit-only vs ${ezDigitPct}% in EZLynx — carrier prefix present in one system but not the other.`
                              : pdfPrefixes.length && !pdfPrefixes.some(p => ezPrefixes.includes(p))
                              ? `Prefix mismatch: PDF uses prefixes [${pdfPrefixes.join(", ")}] but EZLynx uses [${ezPrefixes.join(", ") || "none"}] — different naming convention.`
                              : "No dominant structural gap detected in sampled data. Increase sample size or check for trailing suffix differences."}
                          </p>
                        </div>
                      </section>
                    </div>
